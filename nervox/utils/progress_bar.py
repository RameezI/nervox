...
class ProgressBar:
    _proto_train = (
        "Epoch: {epoch:<{digits_epoch}} "
        "[{percent_complete:>3}% == {samples_done:>{digits_samples}}"
        " samples - {time_span}]"
    )

    _proto_eval = "Eval: [{samples_done:>{digits_samples}}" " samples - {time_span}]"
    ...
    def __init__(self):
        ...
        self._statement_train: str = self._proto_train.format(
            epoch=self.epoch,
        ...
        self._statement_eval: str = self._proto_eval.format(
            samples_done=self._eval_samples_done,
    ...
    def percent_complete(self):
        return (
            np.nan
            if any(np.isnan([self.train_samples, self._train_samples_done]))
            else int(min(self._train_samples_done * 100.0 / self.train_samples, 100.0))
        )
    ...
    def digits_train_samples(self):
        return (
            len(str(100))
            if np.isnan(self.train_samples)
    ...
    def digits_epoch(self):
        return (
            len(str(1000)) if np.isnan(self.max_epochs) else len(str(self.max_epochs))
        )
    ...
    def ckpt_statement(self, value: Dict[str, bool]):
        self._checkpoint = "checkpoint!" if value.get("is_ckpt", False) else ""
    ...
    def __repr__(self):
        if self.mode == ModeProgressBar.DEFAULT:
            ...
            if self.eval_metrics:
                statements.append(
                    re.sub(self._nan_pattern, "?? ", str(self.eval_statement))
                )
    ...
    def __str__(self):
        statement = repr(self)
        statement = (
            self.collapsed_statement(statement, self.terminal_size)
            if self.terminal_size < len(statement)
            else statement
        )
        return statement
    ...
    def collapsed_statement(statement, prescribed_len, sep=" || ", filler=" ... "):
        ...
        def collapsed_segment(segment_statement, budget, segment_sep=" | "):
            ...
            for segment in segments[::-1]:
                seg_char_len = (
                    len(sep.join(segments_to_print)) + len(segment_sep) + len(segment)
                )
                if seg_char_len < budget:
                    segments_to_print.insert(1, segment)
                else:
                    segments_to_print = (
                        [""] if len(segments_to_print) <= 1 else segments_to_print
                    )
                    break
        ...
        for k, split in enumerate(splits[::-1]):
            statement_char_len = (
                len(sep.join(splits_to_print)) + 2 * len(sep) + len(filler) + len(split)
            )
            if statement_char_len < prescribed_len:
                ...
                if k == len(splits) - 1:
                    (
                        ...
                        if not seg_collapsed
                        else splits_to_print[1].rjust(
