...
class DataStream:
    ...
    def __init__(
        ...
        **kwargs_tfds,
        ...
        datasets_dir: str = (
            str(Path(Path.home(), "tensorflow_datasets"))
            if datasets_dir is None
            else str(datasets_dir)
        )
        ...
        if isinstance(dataset, str):
            ...
            self._stream, self._info = tfds.load(
            ...
            none_indexes = [
                k
                for element_spec in tf.nest.flatten(dataset.element_spec)
                for k, dim in enumerate(element_spec.shape)
                if dim is None
            ]
            is_batched = True if none_indexes and len(set(none_indexes)) == 1 else False
            dataset = (
                dataset.unbatch() if is_batched and batch_size is not None else dataset
            )
    ...
    def cardinality(self):
        return (
            ...
            else tf.data.experimental.cardinality(self._stream)
        )

    def __str__(self):
        config = getattr(self, "params")
        ds_name = config["dataset"] if isinstance(config["dataset"], str) else ""
        return ds_name
    ...
    def distribute(self):
        ...
        ), f"Can not make a distributed data stream from {self._stream} object"
        distributed_data_stream = distribution_strategy.experimental_distribute_dataset(
            self._stream
        )
