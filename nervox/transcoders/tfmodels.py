...
def to_tfrt(run_dir, quantization_modes=('JIT_FLOAT16',)):
    ...
    if {'INT8', 'FLOAT16', 'FLOAT32'}.intersection(quantization_modes):
        ...
        assert os.path.isfile(params_file), "parameters file, '{}' ,  does not exist".format(params_file)
        ...
        if {'INT8'}.issubset(quantization_modes):
            params = tf.experimental.tensorrt.ConversionParams(precision_mode='INT8',
        ...
        if {'FLOAT16'}.issubset(quantization_modes):
            ...
            converter = tf.experimental.tensorrt.Converter(input_saved_model_dir=export_dir,
                                                           conversion_params=params)
            converter.convert()
            converter.build(input_fn=data_generator)                            # Generate corresponding TRT engines
            converter.save(os.path.join(export_dir, 'tensorRT', 'float16'))     # Generated engines will be saved.
...
def convert_to_tflite(run_dir, quantization_mode=None, name='model'):
    ...
    concrete_function = imported_model.signatures[tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY]
    ...
    if 'DYNAMIC_INT8' in quantization_mode:
        ...
        with open(os.path.join(export_dir, '{}.tflite'.format(_name)), 'wb') as flat_buffer:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="command line arguments for the converter")
    ...
    parser.add_argument('--logs_dir', required=False, type=str, default=default_log_dir,
                        help='path to the top level tensorflow logs directory')
    parser.add_argument('--name', required=False, type=str, default=default_name,
                        help='name of the experiment directory that contains the runs')
    parser.add_argument('--run_id', required=False, type=str, default=default_run_id,
