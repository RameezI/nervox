...
class Exporter(tf.Module):
    ...
    def __init__(self, models, objectives, **kwargs):
        super().__init__(kwargs.pop('name', None))
        self.models = models
        self.objectives = objectives
    
    @tf.function(input_signature={'image': tf.TensorSpec(shape=None, dtype=tf.float32),
...
# class Exporter:
#     def __init__(self, job_dir: os.PathLike, ckpt_version: Optional[str] = None):
#
...
#         input_tensor_specs = input_tensor_specs[0] if len(input_tensor_specs) == 1 else input_tensor_specs
