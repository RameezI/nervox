...
class ConvNDTranspose(base.Module):
    ...
    def __init__(
        ...
        if padding == "SAME" or padding == "VALID":
            ...
            raise TypeError(
                "ConvNDTranspose only takes string padding, please "
    ...
    def _make_w(self):
        ...
        kernel_shape = utils.replicate(
            self._kernel_shape, self._num_spatial_dims, "kernel_shape"
        )
    ...
    def _get_output_shape(self, inputs):
        input_shape = (
            inputs.shape if inputs.shape.is_fully_defined() else tf.shape(inputs)
        )
        ...
        if self._padding == "VALID":
            ...
            output_shape = smart_lambda(
                lambda x, y: x + y - 1, output_shape, effective_kernel_shape
            )
