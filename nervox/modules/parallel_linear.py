...
class ParallelLinears(base.Module):
    ...
    def _initialize(self, inputs: tf.Tensor):
        ...
        if self.w_init is None:
            ...
            stddev = 1.0 / math.sqrt(self.input_size)
            self.w_init = initializers.TruncatedNormal(stddev=stddev)

        self.w = tf.Variable(
            self.w_init([num_linears, self.input_size, self.output_size], inputs.dtype),
            name="w",
        )

        if self.with_bias:
            self.b = tf.Variable(
                self.b_init([num_linears, 1, self.output_size], inputs.dtype), name="b"
            )
