...
def replicate(
    ...
    raise TypeError(
        "{} must be a scalar or sequence of length 1 or sequence of length {}.".format(
            name, num_times
        )
    )
...
def decorator(
    decorator_fn: Callable[[T, Any, Sequence[Any], Dict[str, Any]], Any],
) -> T:
...
def variable_like(
    inputs: Union[tf.Tensor, tf.Variable],
...
def _render_spec(shape: tf.TensorShape, dtype: tf.DType) -> str:
    ...
    return "{dtype}[{shape}]".format(
        dtype=format_map.get(dtype, dtype.name), shape=",".join(str(d) for d in shape)
    )
...
def format_variables(variables: Sequence[tf.Variable], tablefmt: str = "orgtbl") -> str:
    ...
    return tabulate.tabulate(
        rows, headers=("Variable", "Spec", "Trainable", "Device"), tablefmt=tablefmt
    )
