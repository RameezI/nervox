...
class LayerNorm(Module):
    ...
    def __init__(
        ...
        create_scale: bool,
        create_offset: bool,
        eps: Union[float, TensorLike] = 1e-5,
        ...
        r"""Constructs a ``LayerNorm`` module.
        Args:
    ...
    def build(self, input_shape):
        ...
        if self._channel_index == -1:
            ...
            params_shape = [input_shape[1]] + [1] * (self._rank - 2)

        if self._create_scale:
            self.scale = tf.Variable(
                self._scale_init(params_shape, self.dtype), name="scale"
            )
        else:
            self.scale = None
        ...
        if self._create_offset:
            self.offset = tf.Variable(
                self._offset_init(params_shape, self.dtype), name="offset"
            )
        else:
            self.offset = None
...
class InstanceNorm(LayerNorm):
    ...
    def __init__(
        ...
        Args:
          ...
          name: Name of the module.
        """
        if get_channel_index(data_format) == 1:
            axis = slice(2, None)
