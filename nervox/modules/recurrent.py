...
class UnrolledRNN(base.Module, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def __call__(
        self, input_sequence: types.TensorNest, initial_state: types.TensorNest
    ):
...
class TrainableState(base.Module):
    ...
    def for_core(
        ...
        Returns:
          A `TrainableState`.
        """
        initial_values = tree.map_structure(
            lambda s: tf.squeeze(s, axis=0), core.initial_state(batch_size=1)
        )
...
def static_unroll(
    ...
    for t in range(num_steps):
        outputs, state = _rnn_step(
            core, input_tas, sequence_length, t, prev_outputs=outputs, prev_state=state
        )
        if t == 0:
            output_accs = tree.map_structure(lambda o: _ListWrapper([o]), outputs)
        else:
            tree.map_structure(lambda acc, o: acc.data.append(o), output_accs, outputs)

    output_sequence = tree.map_structure(lambda acc: tf.stack(acc.data), output_accs)
...
class VanillaRNN(RNNCore):
    ...
    def __call__(
        self, inputs: types.TensorNest, prev_state: types.TensorNest
    ) -> Tuple[tf.Tensor, tf.Tensor]:
        """See base class."""
        self._initialize(inputs)

        outputs = self._activation(
            self._input_to_hidden(inputs) + self._hidden_to_hidden(prev_state) + self._b
        )
        ...
        for idx, layer in enumerate(self._layers):
            ...
            if isinstance(layer, RNNCore):
                current_inputs, next_state = layer(
                    current_inputs, prev_state[recurrent_idx]
                )
...
def deep_rnn_with_skip_connections(
    ...
    name: str = "deep_rnn_with_skip_connections",
) -> RNNCore:
...
def deep_rnn_with_residual_connections(
    layers: Sequence[RNNCore], name: str = "deep_rnn_with_residual_connections"
) -> RNNCore:
    ...
    return _LegacyDeepRNN(
        [_ResidualWrapper(l) for l in layers], skip_connections=False, name=name
    )
  ...
  """
    ...
    def __init__(
        ...
        if projection_size is None and projection_init is not None:
            raise ValueError("projection_init must be None when projection is not used")
    ...
    def __call__(self, inputs, prev_state):
        ...
        self._initialize(inputs)
        return _lstm_fn(
            inputs, prev_state, self._w_i, self._w_h, self.b, self.projection
        )
    ...
    def _initialize(self, inputs):
        ...
        self._w_h = tf.Variable(
            w_h_init([self._eff_hidden_size, 4 * self._hidden_size], dtype), name="w_h"
        )
...
class UnrolledLSTM(UnrolledRNN):
    ...
    def __call__(self, input_sequence, initial_state):
        ...
        return _specialized_unrolled_lstm(
            input_sequence, initial_state, self._w_i, self._w_h, self.b
        )
...
class _RecurrentDropoutWrapper(RNNCore):
    ...
    def __call__(self, inputs, prev_state):
        ...
        prev_core_state = tree.map_structure(
            lambda s, mask: (
                s if mask is None else s * mask  # pylint: disable=g-long-lambda
            ),
            prev_core_state,
            dropout_masks,
        )
    ...
    def initial_state(self, batch_size, **kwargs):
        ...
        dropout_masks = tree.map_structure(
            maybe_dropout, core_initial_state, self._rates
        )
...
def lstm_with_recurrent_dropout(hidden_size, dropout=0.5, seed=None, **kwargs):
  ...
  """
    if dropout < 0 or dropout >= 1:
        raise ValueError("dropout must be in the range [0, 1), got {}".format(dropout))
...
class _ConvNDLSTM(RNNCore):
  ...
  """
    ...
    def __init__(
        ...
        self._channel_index = (
            1 if (data_format is not None and data_format.startswith("NC")) else -1
        )
...
def _check_inputs_dtype(inputs, expected_dtype):
    if inputs.dtype is not expected_dtype:
        raise TypeError(
            "inputs must have dtype {!r}, got {!r}".format(expected_dtype, inputs.dtype)
        )
