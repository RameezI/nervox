...
def block(inputs, activation_fn=swish, drop_rate=0., name='',
          ...
          expand_ratio=1, se_ratio=0., id_skip=True, data_format='channels_last'):
    ...
    if strides == 2:
        x = layers.ZeroPadding2D(padding=correct_pad(backend, x, kernel_size), name=name + 'dwconv_pad')(x)
        conv_pad = 'valid'
    else:
        conv_pad = 'same'
