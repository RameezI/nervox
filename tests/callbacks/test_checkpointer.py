...
class TestCheckPointer(tf.test.TestCase):
    ...
    def create_trainer(
        self, model_type, logs_dir, run_id
        ...
        trainer.spin(
            strategy,
            max_epochs=1,
            verbose=VerbosityLevel.KEEP_SILENT,
    ...
    def test_models_uniqueness(self, model):
        ...
        # model-A
        with tempfile.TemporaryDirectory() as temp_dir:
            _, (_, models_A, _) = self.create_trainer(
                model, temp_dir, run_id=str(uuid.uuid4())
            )

        # model-B
        with tempfile.TemporaryDirectory() as temp_dir:
            _, (_, models_B, _) = self.create_trainer(
                model, temp_dir, run_id=str(uuid.uuid4())
            )
    ...
    def test_model_returnability(self, model):
        run_id = str(uuid.uuid4())
        with tempfile.TemporaryDirectory() as temp_dir:
            _, (_, saved_models, _) = self.create_trainer(
                model, temp_dir, run_id=run_id
            )
    ...
    def test_optimizer_returnability(self, model):
        run_id = str(uuid.uuid4())
        with tempfile.TemporaryDirectory() as temp_dir:
            _, (_, saved_models, saved_optimizer) = self.create_trainer(
                model, temp_dir, run_id=run_id
            )
            _, (_, models, optimizer) = self.create_trainer(
                model, temp_dir, run_id=run_id
            )
            ...
            trackables = [
                child
                for child in saved_optimizer._trackable_children()
                if getattr(saved_optimizer, child, None) is not None
            ]

            variables_ref = [
                getattr(saved_optimizer, attribute) for attribute in trackables
            ]

            variables = [getattr(optimizer, attribute) for attribute in trackables]
            ...
            self.assertEqual(optimizer.iterations, saved_optimizer.iterations)
