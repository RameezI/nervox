...
class TestTrainer(tf.test.TestCase):
    ...
    def test_stream_connections(self):
        ...
        trainer = Trainer(self.dummy_train_stream, eval_stream=self.dummy_test_stream)
        ...
        self.assertEqual(
            self.dummy_train_stream.as_dataset(),
        ...
        self.assertEqual(
            self.dummy_test_stream.as_dataset(),
    ...
    def test_nameless_model_addition(self):
        ...
        trainer.push_module("convnet")
        self.assertTrue(
            all(key in trainer.models.keys() for key in ["model", "model_1"])
        )
    ...
    def test_spinning_invocation(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            trainer.push_module("convnet", alias="encoder")
            trainer.push_module(
                GlobalAvgPoolDecoder, alias="decoder", config={"output_units": 101}
            )
    ...
    def test_transfer_learning_keras(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            trainer.push_module(
                DenseNet121,
                alias="encoder",
                config={"trainable": False, "weights": "imagenet"},
            )
            trainer.push_module(
                GlobalAvgPoolDecoder, alias="decoder", config={"output_units": 101}
            )
            ...
            weights_pre_load = [tf.constant(var) for var in model.variables]
            ...
            weights_post_load = [tf.constant(var) for var in model.variables]
            ...
            strategy = Classification(supervision_keys=("image", "label"))
            trainer.spin(strategy, max_epochs=1)
            weights_post_spin = [tf.constant(var) for var in model.variables]
