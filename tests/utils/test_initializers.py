...
"""Tests for various initializers for nervox modules."""
...
class InitializersTest(tf.test.TestCase, parameterized.TestCase):
    def assertDifferentInitializerValues(self, init, shape=None, dtype=tf.float32):
        if shape is None:
            shape = (100,)
...
class ConstantInitializersTest(InitializersTest):
    ...
    @parameterized.parameters((42, tf.float32), (42.0, tf.float32), (42, tf.int32))
...
class RandomUniformInitializerTest(InitializersTest):
    def testRangeInitializer(self):
        shape = (16, 8, 128)
        self.assertRange(
            initializers.RandomUniform(minval=-1.0, maxval=1.0, seed=124.0),
            shape,
            target_mean=0.0,
            target_max=1,
            target_min=-1,
...
class RandomNormalInitializerTest(InitializersTest):
    ...
    def testInvalidDataType(self, dtype):
        init = initializers.RandomNormal(0.0, 1.0)
        with self.assertRaisesRegex(ValueError, r"Expected floating point type, got "):
            init([1], dtype=dtype)
...
class IdentityInitializerTest(InitializersTest):
    @parameterized.parameters(
        *itertools.product(
            [(4, 5), (3, 3), (3, 4, 5), (6, 2, 3, 3)], [3, 1], [tf.float32, tf.int32]
        )
    )
...
class OrthogonalInitializerTest(InitializersTest):
    def testRangeInitializer(self):
        self.assertRange(
            initializers.Orthogonal(seed=123), shape=(20, 20), target_mean=0.0
        )
...
class VarianceScalingInitializerTest(InitializersTest):
    def testTruncatedNormalDistribution(self):
        ...
        self.assertRange(
            init, shape=shape, target_mean=0.0, target_std=1.0 / np.sqrt(shape[0])
        )
    ...
    def test_GlorotNormal(self):
        ...
        self.assertRange(
            initializers.VarianceScaling(
                scale=1.0, mode="fan_avg", distribution="truncated_normal", seed=123
            ),
            shape,
            target_mean=0.0,
            target_std=std,
    ...
    def testLecunNormal(self):
        ...
        self.assertRange(
            initializers.VarianceScaling(
                scale=1.0, mode="fan_in", distribution="truncated_normal", seed=123
            ),
            shape,
            target_mean=0.0,
            target_std=std,
    ...
    def testHeNormal(self):
        ...
        self.assertRange(
            initializers.VarianceScaling(
                scale=2.0, mode="fan_in", distribution="truncated_normal", seed=123
            ),
            shape,
            target_mean=0.0,
            target_std=std,
    ...
    @parameterized.parameters(
        itertools.product(
            ["fan_in", "fan_out", "fan_avg"], ["uniform", "truncated_normal", "normal"]
        )
    )
    ...
    def testCheckInitializersInvalidType(self):
        with self.assertRaisesRegex(
            TypeError, "Initializers must be a dict-like object."
        ):
            initializers.check_initializers([1, 2, 3], ("a"))

    def testCheckInitializersEmpty(self):
