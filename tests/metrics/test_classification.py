...
class TestConfusionVariables(tf.test.TestCase):
    def test_single_sample_unweighted(self):
        ...
        y_pred = tf.constant([[0.3, 0.2, 0.6, 0.9]])

        confusion_matrix, thresholds = compute_confusion_matrix(y_true, y_pred, 1)
        self.assertAllClose([0.0, 0.5, 1.0], thresholds)

        # ture positives
        expectations = [
            [0, 1, 0, 1.0],  # @threshold =  0.0
            [0, 0, 0, 1.0],  # @thresholds = 0.5
        ...
        # false positives
        expectations = [
            [1, 0, 1, 0.0],  # @threshold = 0.0
            [0, 0, 1, 0.0],  # @threshold = 0.5
    ...
    def test_single_sample_classWeights(self):
        ...
        confusion_matrix, thresholds = compute_confusion_matrix(
            y_true, y_pred, bifurcators=1, weights=weights
        )
        self.assertAllClose([0.0, 0.5, 1.0], thresholds)

        # ture positives
        expectations = [
            [0, 1, 0, 2.0],  # @threshold =  0.0
            [0, 0, 0, 2.0],  # @thresholds = 0.5
            [0, 0, 0, 0.0],  # @thresholds = 1.0
        ]
        results = confusion_matrix["true_positives"]
        self.assertAllEqual(expectations, results)
        ...
        # false positives
        expectations = [
            [3.0, 0, 10.0, 0.0],  # @threshold  =  0.0
            [0, 0, 10.0, 0.0],  # @thresholds = 0.5
            [0, 0, 0, 0.0],  # @thresholds = 1.0
        ]
        results = confusion_matrix["false_positives"]
        self.assertAllEqual(expectations, results)
        ...
        # true negatives
        expectations = [
            [0.0, 0.0, 0.0, 0.0],  # @threshold =  0.0
            [3.0, 0.0, 0.0, 0.0],  # @thresholds = 0.5
            [3.0, 0.0, 10.0, 0],  # @thresholds = 1.0
        ]
        results = confusion_matrix["true_negatives"]
        self.assertAllEqual(expectations, results)
        ...
        # false negatives
        expectations = [
            [0, 0.0, 0.0, 0.0],  # @threshold =  0.0
            [0, 1.0, 0.0, 0.0],  # @thresholds = 0.5
            [0, 1.0, 0.0, 2.0],  # @thresholds = 1.0
        ]
        results = confusion_matrix["false_negatives"]
        self.assertAllEqual(expectations, results)
    ...
    def test_multi_sample_unweighted(self):
        ...
        confusion_matrix, thresholds = compute_confusion_matrix(
            y_true, y_pred, bifurcators=1
        )

        # true positives
        expectations = [
            [1, 3, 2, 3.0],  # @threshold =  0.0
            [0, 0, 1, 3.0],  # @thresholds = 0.5
            [0, 0, 0, 0],  # @thresholds = 1.0
        ]
        results = confusion_matrix["true_positives"]
        self.assertAllEqual(expectations, results)
...
class TestAccuracyScore(tf.test.TestCase):
    def setUp(self):
        self.labels = tf.stack(
            [
                ...
                tf.one_hot(1, 4),
                tf.constant([1.0, 1.0, 0, 0.0]),
            ],
            axis=0,
        )
        self.scores = tf.stack(
            [
                tf.one_hot(0, 4),
                tf.constant([1.0, 0, 0.0, 0.0]),
                tf.one_hot(2, 4),
    ...
    def test_microAverage_with_sample_weights(self):
        ...
        labels, scores = (self.labels, self.scores)
        expectation = tf.constant(9 / 12.0)
        accuracy_score = AccuracyScore(averaging_mode=AveragingMode.MICRO)
        accuracy_score.update(labels, scores, weights=[0, 1, 1, 1])
        self.assertAllClose(expectation, accuracy_score.result())
        accuracy_score.reset()
    ...
    def test_macroAverage_unweighted(self):
        labels, scores = self.labels, self.scores
        expectation = tf.constant(13.0 / 16)
...
class TestPrecisionScore(tf.test.TestCase):
    ...
    def test_macroAverage_unweighted(self):
        ...
        precision_score.update(labels, scores)
        expectation = tf.constant(6 / 16.0)
        result = precision_score.result()
        self.assertAllClose(expectation, result)
...
class TestAveragePrecisionScore(tf.test.TestCase):
    ...
    def test_microAverage_unweighted(self):
        ...
        result = mAP.result()
        expectation = average_precision_score(labels, scores, average="micro")
        self.assertAllClose(expectation, result)

        mAP = AveragePrecisionScore(
            transform=tf.nn.sigmoid, averaging_mode=AveragingMode.MICRO, bifurcators=500
        )
    ...
    def test_weightedAverage_unweighted(self):
        labels, scores = self.labels, self.scores
        mAP = AveragePrecisionScore(
            averaging_mode=AveragingMode.WEIGHTED, bifurcators=500
        )
        mAP.update(labels, scores)
        result = mAP.result()
